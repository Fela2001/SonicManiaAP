...
LOCATION_NAME_TO_ID = {
    ...
    "Hydro City Act 1 Clear": 13,
    "Hydro City Act 2 Clear": 14,
    "Mirage Saloon ST Act 1 Clear": 15,
...
def create_regular_locations(world: SonicManiaPlusWorld) -> None:
    ...
    green_hill_locations = get_location_names_with_ids(
        ["Green Hill Act 1 Clear", "Green Hill Act 2 Clear", "Special Ring near Knuckles Start in Green Hill Act 1", "Special Ring in hidden waterfall cave in Green Hill Act 1", 
    ...
    studiopolis_locations = get_location_names_with_ids(
        ["Studiopolis Act 1 Clear", "Studiopolis Act 2 Clear", "Special Ring near the start of Studiopolis Act 1 behind yellow lit windows", "Special Ring right above a Analog TV and right before a loop-de-loop in Studiopolis Act 1", 
    ...
    flying_battery_locations = get_location_names_with_ids(
        ["Flying Battery Act 1 Clear", "Flying Battery Act 2 Clear", "Special Ring hidden in leftward pathway reached by electric shield attraction or flight in Flying Battery Act 1", 
    ...
    press_garden_locations = get_location_names_with_ids(
        ["Press Garden Act 1 Clear", "Press Garden Act 2 Clear", "Special Ring on the lower left near the start of Press Garden Act 1", 
    ...
    stardust_speedway_locations = get_location_names_with_ids(
        ["Stardust Speedway Act 1 Clear", "Stardust Speedway Act 2 Clear", "Special Ring between two pillars on the upper path early on in Stardust Speedway Act 1", 
    ...
    hydro_city_locations = get_location_names_with_ids(
        ["Hydro City Act 1 Clear", "Hydro City Act 2 Clear", "Special Ring above hand speed launcher and a ramp in Hydro City Act 1", 
    ...
    mirage_saloon_locations = get_location_names_with_ids(
        ["Mirage Saloon ST Act 1 Clear", "Mirage Saloon K Act 1 Clear", "Mirage Saloon Act 2 Clear", "Special Ring at left end of Eggman's train in Mirage Saloon ST Act 1", 
    ...
    oil_ocean_locations = get_location_names_with_ids(
        ["Oil Ocean Act 1 Clear", "Oil Ocean Act 2 Clear", "Special Ring near the start of Oil Ocean Act 1 that is above five fans", 
    ...
    lava_reef_locations = get_location_names_with_ids(
        ["Lava Reef Act 1 Clear", "Lava Reef Act 2 Clear", "Special Ring hidden to the left of the first rising and falling spike crusher in Lava Reef Act 1",
    ...
    metallic_madness_locations = get_location_names_with_ids(
        ["Metallic Madness Act 1 Clear", "Metallic Madness Act 2 Clear", "Special Ring reached through bouncing on a red spring to run a half loop to the ring in Metallic Madness Act 1", 
